...
export const BackgroundBeams = React.memo(
  ({ className }: { className?: string }) => {
    ...
    // Animate gradients with vanilla rAF — production-safe
    useEffect(() => {
      const svg = svgRef.current;
      if (!svg) return;

      const gradients = svg.querySelectorAll<SVGLinearGradientElement>(
        "linearGradient[data-animated]"
      );
      if (gradients.length === 0) return;

      // Pre-compute per-gradient animation config
      const configs = Array.from(gradients).map(() => {
        const duration = (Math.random() * 10 + 10) * 1000; // 10–20s in ms
        const delay = Math.random() * 10 * 1000; // 0–10s delay
        const y2End = 93 + Math.random() * 8;
        return { duration, delay, y2End };
      });
      ...
      function tick(now: number) {
        ...
        gradients.forEach((g, i) => {
          const { duration, delay, y2End } = configs[i];
          const localTime = elapsed - delay;
          if (localTime < 0) return; // still waiting for delay

          // Ping-pong progress: 0→1→0→1…
          const rawProgress = (localTime % duration) / duration;
          const progress =
            rawProgress <= 0.5
              ? rawProgress * 2
              : 2 - rawProgress * 2;
          ...
          g.setAttribute("x1", `${eased * 100}%`);
          g.setAttribute("x2", `${eased * 95}%`);
          g.setAttribute("y1", `${eased * 100}%`);
          g.setAttribute("y2", `${eased * y2End}%`);
        });
    ...
    return (
      <div
        className={cn(
          "absolute inset-y-0 w-screen left-1/2 -translate-x-1/2 flex items-center justify-center overflow-hidden",
          className,
        )}
        style={{
      ...
      >
        <svg
          ref={svgRef}
          className="pointer-events-none absolute z-0 h-full w-full overflow-visible"
          width="100%"
          height="100%"
          viewBox="0 0 696 316"
          fill="none"
        ...
        >
          ...
          <defs>
            {/* Animated gradients — driven by rAF, not Framer Motion */}
